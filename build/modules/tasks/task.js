  ...
  wrapper.innerHTML = `
    <div class="task group relative">
      ...
      <div class="relative">
        ...
        <div
          id="task-dropdown"
          class="flex absolute inset-e-5 top-0 mt-1 w-28 rounded-lg border border-neutral-200 dark:border-[#3D3D3D] bg-card-light dark:bg-[#0B192D] shadow-[0px_12px_24px_-6px_#1414190F] z-10 overflow-hidden items-center py-2"
        >
          <button
            type="button"
            id="task-edit-btn"
...
export function renderTaskList(containerSelector) {
  ...
  tasks.forEach((task) => {
    const itemEl = renderTaskItem(task);
